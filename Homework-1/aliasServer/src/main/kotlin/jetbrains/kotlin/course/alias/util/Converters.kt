...
fun Card.toJsCard(): JsCard = JsCard(this.id, this.words.map { it.content }.toTypedArray())
...
fun GameJsResult.toGameResult(teamService: TeamService): GameResult = this.map {
    val team = teamService.getTeamById(it.id)
        ?: error("Internal error! Unknown team with id: ${it.id} was received!")

    team.copy(points = it.points)
}
